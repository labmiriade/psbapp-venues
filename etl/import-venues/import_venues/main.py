...
def put_places(csvfile, batch) -> Tuple[Set[str], Set[str], List[Exception]]:
    ...
    for row in reader:
        ...
        try:
            batch.put_item(
                Item={
                    ...
                    "data": {
                        ...
                        "category": get_or_error(row, "Tipo").strip().lower(),
                        ...
                        "description": get_or_error(row, "Descrizione"),
                        "availability": get_or_error(row, "Disponibilita"),
                        "equipment": get_or_error(row, "Attrezzatura_tecnica"),
                        "maxCapacity": get_or_error(row, "Capacita_max"),
                        "size": get_or_error(row, "Dimensione_mq"),
                        "collocation": get_or_error(row, "Collocazione"),
                        "accessibility": get_or_error(row, "Accessibilita"),
                        "wifi": get_or_error(row, "Presenza_Wi-fi"),
                        "website": get_or_error(row, "Sito_web"),
                        "contact": get_or_error(row, "Contatto"),
                        "emailAddress": get_or_error(row, "Contatto_Email"),                        
            ...
            ids.add(place_id)
            if row["tipo"]:
                categories.add(row["tipo"].strip().lower())
        except Exception as error:
            errors.append(error)
