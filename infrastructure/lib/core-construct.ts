...
export interface CoreConstructProps {
  ...
  /**
   * The territory venues csv data urls
   */
  csvDataUrls: string;
  /**
...
export class CoreConstruct extends Construct {
  constructor(scope: Construct, id: string, props: CoreConstructProps) {
    ...
    // lambda to import territory venues to dynamodb
    const venuesImportLambda = new lambda.Function(this, 'VenuesImportFn', {
      code: new lambda.AssetCode('../etl/import-venues', {
        bundling: {
      ...
      memorySize: 512,
      description: 'Lambda per importare luoghi eventi',
      environment: {
        DATA_TABLE: dataTable.tableName,
        CSV_DATA_URLS: props.csvDataUrls,
      },
    });
    dataTable.grantReadWriteData(venuesImportLambda);

    const venuesImportErrors = venuesImportLambda.metricErrors({
      period: cdk.Duration.minutes(1),
    });

    const importAlarm = new cloudwatch.Alarm(this, 'venuesImportErrorsAlarm', {
      metric: venuesImportErrors,
      threshold: 1,
      comparisonOperator: cloudwatch.ComparisonOperator.GREATER_THAN_OR_EQUAL_TO_THRESHOLD,
      evaluationPeriods: 1,
      alarmDescription: 'An error occurred during the VenuesImport function execution',
    });
    ...
    eventRule.addTarget(new targets.LambdaFunction(venuesImportLambda));
